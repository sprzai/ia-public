...
class PropertyScraper:
    ...
    def scrape_page(self, url):
        ...
        for item in items:
            ...
            if currency_element and price_element and len(attributes) >= 3:
                ...
                bedrooms = self.extract_number(attributes[0].text)
                bathrooms = self.extract_number(attributes[1].text)
                area = self.extract_number(attributes[2].text)
    ...
    def scrape(self, start_page, end_page):
        ...
        for page in range(start_page, end_page + 1):
            url = f"{self.base_url}_Desde_{(page)*49}_OrderId_PRICE_NoIndex_True"
        ...
        print(f"Los datos se han guardado en '{self.filename}'")


base_url = "https://www.portalinmobiliario.com/venta/casa/valdivia-de-los-rios/"
filename = "propiedades_valdivia.csv"

scraper = PropertyScraper(base_url, filename)
